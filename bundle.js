...
__export(src_exports, {
  ...
  modelToTiktokenModel: () => modelToTiktokenModel,
...
var scalarTypes = exhaustiveTuple()("any", "boolean", "string", "number", "date", "time", "datetime", "chat-message", "control-flow-excluded", "object", "gpt-tool", "vector");
...
var ExtractRegexNodeImpl = class extends NodeImpl {
  ...
  async process(inputs) {
    ...
    let match8;
    let firstMatch;
    while ((match8 = regExp.exec(inputString)) !== null) {
      if (!firstMatch) {
        firstMatch = match8;
      }
      matches.push(match8[1]);
    }
...
var MatchNodeImpl = class extends NodeImpl {
  ...
  async process(inputs) {
    ...
    for (let i = 0; i < cases.length; i++) {
      ...
      const match8 = regExp.test(inputString);
      if (match8) {
        matched = true;
        output[`case${i + 1}`] = {
          type: "string",
...
var ExtractYamlNodeImpl = class extends NodeImpl {
  ...
  async process(inputs) {
    var _a, _b;
    const inputString = expectType(inputs["input"], "string");
    const match8 = new RegExp(`^${this.data.rootPropertyName}:`, "m").exec(inputString);
    const rootPropertyStart = (match8 == null ? void 0 : match8.index) ?? -1;
    ...
    if (this.data.objectPath) {
      try {
        const extractedValue = (0, import_jsonpath_plus.JSONPath)({ json: yamlObject, path: this.data.objectPath });
...
var ExtractObjectPathNodeImpl = class extends NodeImpl {
  ...
  async process(inputs) {
    ...
    let matches;
    try {
      matches = (0, import_jsonpath_plus2.JSONPath)({ json: inputObject ?? null, path: inputPath });
    } catch (err) {
      matches = [];
    }
    if (matches.length === 0) {
      ...
      try {
        ...
        if (message.type === "run") {
          ...
          for (const processor of processors) {
            await (0, import_ts_pattern4.match)(message).with({ type: "abort" }, async () => {
...
function getCostForTokens2(tokenCount, type, model) {
  const costPerThousand = modelCost2[model][type];
  return tokenCount / 1e3 * costPerThousand;
}
function getCostForPrompt2(messages, model) {
  const tokenCount = getTokenCountForMessages2(messages, modelToTiktokenModel2[model]);
...
var modelOptions2 = Object.entries(modelDisplayNames2).map(([value, label]) => ({ value, label }));
...
function coerceTypeOptional2(wrapped, type) {
  ...
  const result = (0, import_ts_pattern5.match)(type).with("string", () => coerceToString2(value)).with("boolean", () => coerceToBoolean2(value)).with("chat-message", () => coerceToChatMessage2(value)).with("number", () => coerceToNumber2(value)).with("object", () => coerceToObject2(value)).otherwise(() => {
    if (!value) {
      return value;
    }
...
var ChatNodeImpl2 = class extends NodeImpl2 {
  ...
  async process(inputs, context) {
    ...
    const tools = expectTypeOptional2(inputs["tools"], "gpt-tool[]");
    const toolMap = (tools ?? []).reduce((acc, tool) => {
      if (tool.namespace) {
        const existing = acc[tool.namespace] ?? {};
        const namespace = {
    ...
    let { maxTokens } = this.data;
    const tokenCount = getTokenCountForMessages2(completionMessages, modelToTiktokenModel2[model]);
    if (tokenCount >= modelMaxTokens2[model]) {
      throw new Error(
        `The model ${model} can only handle ${modelMaxTokens2[model]} tokens, but ${tokenCount} were provided in the prompts alone.`
      );
    }
    if (tokenCount + maxTokens > modelMaxTokens2[model]) {
      const message = `The model can only handle a maximum of ${modelMaxTokens2[model]} tokens, but the prompts and max tokens together exceed this limit. The max tokens has been reduced to ${modelMaxTokens2[model] - tokenCount}.`;
      addWarning2(output, message);
      maxTokens = Math.floor((modelMaxTokens2[model] - tokenCount) * 0.95);
    }
    const isMultiResponse = this.data.useNumberOfChoicesInput || (this.data.numberOfChoices ?? 1) > 1;
    try {
      return await pRetry(
        async () => {
          ...
          const requestTokenCount = getTokenCountForMessages2(completionMessages, modelToTiktokenModel2[model]);
          output["requestTokens"] = { type: "number", value: requestTokenCount * numberOfChoices };
          const responseTokenCount = responseChoicesParts.map((choiceParts) => getTokenCountForString2(choiceParts.join(), modelToTiktokenModel2[model])).reduce((a, b) => a + b, 0);
          output["responseTokens"] = { type: "number", value: responseTokenCount };
...
function getChatNodeMessages2(inputs) {
  ...
  let messages = (0, import_ts_pattern6.match)(prompt).with({ type: "chat-message" }, (p) => [p.value]).with({ type: "chat-message[]" }, (p) => p.value).with({ type: "string" }, (p) => [{ type: "user", message: p.value }]).with({ type: "string[]" }, (p) => p.value.map((v) => ({ type: "user", message: v }))).otherwise((p) => {
    ...
    return coercedString != null ? [{ type: "user", message: coerceType2(p, "string") }] : [];
  });
  const systemPrompt = inputs["systemPrompt"];
  if (systemPrompt) {
    messages = [{ type: "system", message: coerceType2(systemPrompt, "string") }, ...messages];
  }
  return { messages, systemPrompt };
}
...
var ChunkNodeImpl2 = class extends NodeImpl2 {
  ...
  async process(inputs) {
    ...
    const chunked = chunkStringByTokenCount2(
      ...
      modelToTiktokenModel2[this.chartNode.data.model],
      overlapPercent
    );
    return {
      ["chunks"]: {
...
var ExtractYamlNodeImpl2 = class extends NodeImpl2 {
  ...
  async process(inputs) {
    ...
    if (this.data.objectPath) {
      try {
        const extractedValue = (0, import_jsonpath_plus3.JSONPath)({ json: yamlObject, path: this.data.objectPath });
        matches = extractedValue;
        yamlObject = extractedValue.length > 0 ? extractedValue[0] : void 0;
      } catch (err) {
        return {
...
var TrimChatMessagesNodeImpl2 = class extends NodeImpl2 {
  ...
  async process(inputs) {
    ...
    const removeFromBeginning = this.chartNode.data.removeFromBeginning;
    const model = "gpt-3.5-turbo";
    const tiktokenModel = modelToTiktokenModel2[model];
    let trimmedMessages = [...input];
...
var ExtractObjectPathNodeImpl2 = class extends NodeImpl2 {
  ...
  async process(inputs) {
    ...
    let matches;
    try {
      matches = (0, import_jsonpath_plus4.JSONPath)({ json: inputObject ?? null, path: inputPath });
    } catch (err) {
      matches = [];
    }
    if (matches.length === 0) {
...
var ToolNodeImpl2 = class extends NodeImpl2 {
  static create() {
    const chartNode = {
      type: "tool",
      title: "Tool",
...
var toolNode2 = nodeDefinition2(ToolNodeImpl2, "Tool");
...
var HashNodeImpl2 = class extends NodeImpl2 {
  ...
  async process(inputs) {
    ...
    const hash = (0, import_ts_pattern7.match)(this.data.algorithm).with("md5", () => (0, import_md52.default)(inputText).toString()).with("sha1", () => (0, import_sha12.default)(inputText).toString()).with("sha256", () => (0, import_sha2562.default)(inputText).toString()).with("sha512", () => (0, import_sha5122.default)(inputText).toString()).exhaustive();
    return {
      ["hash"]: {
        type: "string",
        value: hash
