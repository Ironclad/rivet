...
function getChatNodeMessages2(inputs) {
  ...
  let messages = (0, import_ts_pattern6.match)(prompt).with({ type: "chat-message" }, (p) => [p.value]).with({ type: "chat-message[]" }, (p) => p.value).with({ type: "string" }, (p) => [{ type: "user", message: p.value, function_call: void 0 }]).with(
    { type: "string[]" },
