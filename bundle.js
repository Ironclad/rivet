...
var ControlFlowExcluded = Symbol("ControlFlowExcluded");
var ControlFlowExcludedPort = ControlFlowExcluded;
var Warnings = Symbol("Warnings");
...
var scalarTypes = exhaustiveTuple()("any", "boolean", "string", "number", "date", "time", "datetime", "chat-message", "control-flow-excluded", "object");
...
async function* streamChatCompletions({ auth, signal, model, messages, temperature, top_p, max_tokens, n, stop, presence_penalty, frequency_penalty, logit_bias }) {
  var _a, _b, _c;
  ...
  for await (const chunk of response.events()) {
    ...
    const text = ((_c = (_b = (_a = data == null ? void 0 : data.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.delta) == null ? void 0 : _c.content) ?? "";
...
var ChatNodeImpl = class extends NodeImpl {
  ...
  async process(inputs, context) {
    ...
    const stop = this.data.useStop ? expectTypeOptional(inputs["stop"], "string") ?? this.chartNode.data.stop : void 0;
...
var GraphProcessor = class {
  ...
  constructor(project, graphId) {
    ...
    for (const node of this.#graph.nodes) {
      this.#nodeInstances[node.id] = createNodeInstance(node);
  ...
  async processGraph(context, inputs = {}, contextValues = {}) {
    try {
      ...
      this.#erroredNodes = /* @__PURE__ */ new Set();
      ...
      if (this.#erroredNodes.size > 0) {
        const error = Error(`Graph ${this.#graph.metadata.name} (${this.#graph.metadata.id}) failed to process due to errors in nodes: ${Array.from(this.#erroredNodes).map((nodeId) => `${this.#nodesById[nodeId].title} (${nodeId})`).join(", ")}`);
      ...
      const outputNodes = this.#graph.nodes.filter((node) => this.#isNodeOfType("graphOutput", node) && !this.#excludedDueToControlFlow(node, this.#getInputValuesForNode(node), (0, import_nanoid31.nanoid)()));
      const outputValues = outputNodes.reduce((values2, node) => {
        const results = this.#nodeResults.get(node.id);
        if (results) {
          values2[node.data.id] = results["value"];
        }
        return values2;
      }, {});
      this.#running = false;
  ...
  async #fetchNodeDataAndProcessNode(node, cycleInfo) {
    ...
    cycleInfo == null ? void 0 : cycleInfo.nodes.add(node);
  ...
  async #processNodeIfAllInputsAvailable(node, loopInfo) {
    ...
    if (this.#excludedDueToControlFlow(node, inputValues, (0, import_nanoid31.nanoid)(), "loop-not-broken")) {
    ...
    if (node.type === "loopController") {
      ...
      if (childLoopInfo.iterationCount > (node.data.maxIterations ?? 100)) {
        throw new Error(`Loop controller ${node.title} has exceeded max iterations of ${node.data.maxIterations ?? 100}`);
      }
    }
    this.#processingQueue.addAll(outputNodes.map((outputNode) => async () => {
      ...
      await this.#processNodeIfAllInputsAvailable(outputNode, childLoopInfo);
    }));
  }
  async #processNode(node) {
    const processId = (0, import_nanoid31.nanoid)();
  ...
  async #processGraphInputNode(node, processId) {
    const inputValues = this.#getInputValuesForNode(node);
    if (this.#excludedDueToControlFlow(node, inputValues, processId)) {
      return;
    }
    this.#emitter.emit("nodeStart", { node, inputs: inputValues, processId });
    const impl = this.#nodeInstances[node.id];
    const outputValues = await impl.getOutputValuesFromGraphInput(this.#graphInputs, inputValues);
    this.#nodeResults.set(node.id, outputValues);
    this.#visitedNodes.add(node.id);
    this.#emitter.emit("nodeFinish", { node, outputs: outputValues, processId });
  }
  #processGraphOutputNode(node, processId) {
    const inputValues = this.#getInputValuesForNode(node);
    if (this.#excludedDueToControlFlow(node, inputValues, processId)) {
      return;
    }
  ...
  #excludedDueToControlFlow(node, inputValues, processId, typeOfExclusion = void 0) {
    ...
    const controlFlowExcludedValues = inputValuesList.filter((value) => (value == null ? void 0 : value.type) === "control-flow-excluded" && value.value === typeOfExclusion);
    ...
    const nodesAllowedToConsumeExcludedValue = ["if", "ifElse", "coalesce"];
  ...
  #getInputValuesForNode(node) {
    ...
    return this.#definitions[node.id].inputs.reduce((values2, input) => {
      ...
      if (connection) {
        ...
        const outputResult = (_a = this.#nodeResults.get(outputNode.id)) == null ? void 0 : _a[connection.outputId];
