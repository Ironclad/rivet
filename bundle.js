...
// ../../.yarn/cache/lodash-es-npm-4.17.21-b45832dfce-d1c3ee9411.zip/node_modules/lodash-es/toInteger.js
...
// ../../.yarn/cache/lodash-es-npm-4.17.21-b45832dfce-d1c3ee9411.zip/node_modules/lodash-es/flattenDepth.js
...
function coerceTypeOptional(wrapped, type) {
  ...
  if (isArrayDataType(type) && isArrayDataValue(value) && getScalarTypeOf(type) !== getScalarTypeOf(value.type)) {
    return value.value.map((v) => coerceTypeOptional(inferType(v), getScalarTypeOf(type)));
  }
          ...
          if (Array.isArray(input == null ? void 0 : input.value)) {
            for (const value of (input == null ? void 0 : input.value) ?? []) {
              if (this.data.flattenDeep) {
                outputArray.push(...flattenDepth_default(value));
              } else {
...
// ../core/dist/model/nodes/AbortGraphNode.js
...
// ../core/dist/model/nodes/RaceInputsNode.js
...
var RaceInputsNodeImpl = class extends NodeImpl {
  ...
  async process(inputs, context) {
    const value = Object.entries(inputs).find(([key, value2]) => key.startsWith("input") && value2 !== void 0);
    if (!value) {
      return {
        ["result"]: {
          type: "control-flow-excluded",
...
// ../core/dist/model/nodes/ToJsonNode.js
...
// ../core/dist/model/nodes/JoinNode.js
...
var JoinNodeImpl = class extends NodeImpl {
  ...
  getInputDefinitions(connections) {
    const inputs = [];
    const inputCount = __privateMethod(this, _getInputPortCount6, getInputPortCount_fn6).call(this, connections);
...
__publicField(JoinNodeImpl, "create", () => {
  const chartNode = {
    type: "join",
    title: "Join",
...
// ../core/dist/model/nodes/FilterNode.js
...
var GraphProcessor = class {
  ...
  async processGraph(context, inputs = {}, contextValues = {}) {
    try {
      ...
      if (erroredNodes.length && !this.#abortSuccessfully) {
        const error = this.#abortError ?? Error(`Graph ${this.#graph.metadata.name} (${this.#graph.metadata.id}) failed to process due to errors in nodes: ${erroredNodes.map(([nodeId, error2]) => `${this.#nodesById[nodeId].title} (${nodeId}): ${error2}`).join(", ")}`);
  ...
  async #processNodeIfAllInputsAvailable(node) {
    ...
    if (this.#excludedDueToControlFlow(node, inputValues, (0, import_nanoid43.nanoid)(), "loop-not-broken")) {
    ...
    if (node.type === "raceInputs") {
      const allNodesForRace = [...this.#attachedNodeData.entries()].filter(([, { races }]) => races == null ? void 0 : races.raceIds.includes(`race-${node.id}`));
  ...
  async #processNode(node) {
    const processId = (0, import_nanoid43.nanoid)();
  ...
  #outputNodesFrom(node) {
    ...
    const outputNodes = uniqBy_default(outputConnections.map((conn) => this.#nodesById[conn.inputNodeId]).filter(isNotNull), (x) => x.id);
...
function deserializeProject(serializedProject) {
  try {
    ...
    if (err instanceof yaml3.YAMLError) {
      yamlProblem(err);
    }
    ...
    try {
      ...
      if (err2 instanceof yaml3.YAMLError) {
        yamlProblem(err2);
      }
      console.warn(`Failed to deserialize project v2: ${getError(err2).stack}`);
      try {
        return projectV1Deserializer(serializedProject);
      } catch (err3) {
        console.warn(`Failed to deserialize project v1: ${getError(err3).stack}`);
...
var _ExecutionRecorder = class {
  ...
  record(processor) {
    this.recordingId = (0, import_nanoid45.nanoid)();
    processor.onAny((event, data) => {
...
function coerceTypeOptional2(wrapped, type) {
  ...
  if (isArrayDataType2(type) && isArrayDataValue2(value) && getScalarTypeOf2(type) !== getScalarTypeOf2(value.type)) {
    return value.value.map((v) => coerceTypeOptional2(inferType2(v), getScalarTypeOf2(type)));
  }
...
var LoopControllerNodeImpl2 = class extends NodeImpl2 {
  ...
  getInputDefinitions(connections, nodes) {
    const inputs = [];
    const messageCount = __privateMethod(this, _getInputPortCount8, getInputPortCount_fn8).call(this, connections);
    inputs.push({
      dataType: "any",
      id: "continue",
      title: "Continue"
...
var _getInputPortCount9, getInputPortCount_fn9;
var CoalesceNodeImpl2 = class extends NodeImpl2 {
  constructor() {
    super(...arguments);
    __privateAdd(this, _getInputPortCount9);
  }
  getInputDefinitions(connections) {
    const inputs = [];
    const inputCount = __privateMethod(this, _getInputPortCount9, getInputPortCount_fn9).call(this, connections);
    inputs.push({
      dataType: "boolean",
      id: "conditional",
      title: "Conditional"
...
_getInputPortCount9 = new WeakSet();
getInputPortCount_fn9 = function(connections) {
  const inputNodeId = this.chartNode.id;
  const inputConnections = connections.filter(
    (connection) => connection.inputNodeId === inputNodeId && connection.inputId.startsWith("input")
  );
...
var _getInputPortCount10, getInputPortCount_fn10;
var PassthroughNodeImpl2 = class extends NodeImpl2 {
  constructor() {
    super(...arguments);
    __privateAdd(this, _getInputPortCount10);
  }
  getInputDefinitions(connections) {
    const inputs = [];
    const inputCount = __privateMethod(this, _getInputPortCount10, getInputPortCount_fn10).call(this, connections);
    for (let i = 1; i <= inputCount; i++) {
      inputs.push({
        dataType: "any",
        id: `input${i}`,
...
_getInputPortCount10 = new WeakSet();
getInputPortCount_fn10 = function(connections) {
  const inputNodeId = this.chartNode.id;
  const inputConnections = connections.filter(
    (connection) => connection.inputNodeId === inputNodeId && connection.inputId.startsWith("input")
  );
...
var import_nanoid81 = require("nanoid");
var import_ts_dedent5 = __toESM(require("ts-dedent"));
var VectorNearestNeighborsNodeImpl2 = class extends NodeImpl2 {
  static create() {
    return {
      id: (0, import_nanoid81.nanoid)(),
      type: "vectorNearestNeighbors",
      title: "Vector KNN",
      visualData: { x: 0, y: 0, width: 200 },
      data: {
  ...
  getBody() {
    return import_ts_dedent5.default`
      ${this.data.useIntegrationInput ? "(Integration using input)" : this.data.integration}
      k: ${this.data.useKInput ? "(using input)" : this.data.k}
      ${this.data.useCollectionIdInput ? "(using input)" : this.data.collectionId}
    `;
...
var import_nanoid82 = require("nanoid");
...
var import_nanoid83 = require("nanoid");
var import_ts_dedent6 = __toESM(require("ts-dedent"));
var AbortGraphNodeImpl2 = class extends NodeImpl2 {
  static create() {
    const chartNode = {
      type: "abortGraph",
      title: "Graph Output",
      id: (0, import_nanoid83.nanoid)(),
      visualData: {
        x: 0,
        y: 0,
        width: 200
  ...
  getBody() {
    return import_ts_dedent6.default`
      ${this.data.useSuccessfullyInput ? "Success depends on input" : this.data.successfully ? "Successfully Abort" : this.data.errorMessage ? `Error Abort: ${this.data.errorMessage}` : "Error Abort"}
    `;
  }
  async process(inputs, context) {
    ...
    if (successfully) {
      ...
      const errorMessage = (((_a = coerceTypeOptional2(inputs["data"], "string")) == null ? void 0 : _a.trim()) || this.data.errorMessage) ?? "Graph aborted with error";
      context.abortGraph(errorMessage);
    }
    return {};
  }
...
var import_nanoid84 = require("nanoid");
var _getInputPortCount11, getInputPortCount_fn11;
var RaceInputsNodeImpl2 = class extends NodeImpl2 {
  constructor() {
    super(...arguments);
    __privateAdd(this, _getInputPortCount11);
  }
  static create() {
    const chartNode = {
      type: "raceInputs",
      title: "Race Inputs",
      id: (0, import_nanoid84.nanoid)(),
      visualData: {
        x: 0,
        y: 0,
        width: 300
  ...
  getInputDefinitions(connections) {
    const inputs = [];
    const inputCount = __privateMethod(this, _getInputPortCount11, getInputPortCount_fn11).call(this, connections);
    for (let i = 1; i <= inputCount; i++) {
      inputs.push({
        dataType: "any",
        id: `input${i}`,
...
_getInputPortCount11 = new WeakSet();
getInputPortCount_fn11 = function(connections) {
  const inputNodeId = this.chartNode.id;
  const inputConnections = connections.filter(
    (connection) => connection.inputNodeId === inputNodeId && connection.inputId.startsWith("input")
  );
...
var import_nanoid85 = require("nanoid");
var ToJsonNodeImpl2 = class extends NodeImpl2 {
  static create() {
    const chartNode = {
      type: "toJson",
      title: "To JSON",
      id: (0, import_nanoid85.nanoid)(),
      visualData: {
        x: 0,
        y: 0,
        width: 175
...
var import_nanoid86 = require("nanoid");
var _getInputPortCount12, getInputPortCount_fn12;
var JoinNodeImpl2 = class extends NodeImpl2 {
  constructor() {
    super(...arguments);
    __privateAdd(this, _getInputPortCount12);
  }
  getInputDefinitions(connections) {
    const inputs = [];
    const inputCount = __privateMethod(this, _getInputPortCount12, getInputPortCount_fn12).call(this, connections);
    if (this.data.useJoinStringInput) {
      inputs.push({
        dataType: "string",
        id: "joinString",
...
_getInputPortCount12 = new WeakSet();
getInputPortCount_fn12 = function(connections) {
  const inputNodeId = this.chartNode.id;
  const inputConnections = connections.filter(
    (connection) => connection.inputNodeId === inputNodeId && connection.inputId.startsWith("input")
  );
...
__publicField(JoinNodeImpl2, "create", () => {
  const chartNode = {
    type: "join",
    title: "Join",
    id: (0, import_nanoid86.nanoid)(),
    data: {
      flatten: true,
      joinString: "\n"
    },
...
var import_nanoid87 = require("nanoid");
var FilterNodeImpl2 = class extends NodeImpl2 {
  static create() {
    const chartNode = {
      type: "filter",
      title: "Filter",
      id: (0, import_nanoid87.nanoid)(),
      visualData: {
        x: 0,
        y: 0,
        width: 175
...
var import_nanoid90 = require("nanoid");
