...
import { isArrayDataValue, arrayizeDataValue, } from './DataValue';
...
export class GraphProcessor {
    ...
    contextValues = {}) {
        try {
            ...
            const outputNodes = this.#graph.nodes.filter((node) => this.#isNodeOfType('graphOutput', node) &&
            ...
            const outputValues = outputNodes.reduce((values, node) => {
                ...
                if (results) {
                    values[node.data.id] = results['value'];
                }
                return values;
            }, {});
            this.#running = false;
    ...
    #excludedDueToControlFlow(node, inputValues, processId, typeOfExclusion = undefined) {
        ...
        const controlFlowExcludedValues = inputValuesList.filter((value) => value?.type === 'control-flow-excluded' && value.value === typeOfExclusion);
